...
class WatchLinkService {
    ...
    function initialize(batchQueue, collector) {
        ...
        mShouldExitAfterTransmit = false;
    }
    ...
    function requestSyncNow(reason) {
        emitHandshake();
        requestSyncWithoutHandshake(reason);
    }

    function requestSyncWithoutHandshake(reason) {
    ...
    function enableBackgroundExit() {
        mShouldExitAfterTransmit = true;
    }

    function finishBackgroundCycle() {
        if (!mShouldExitAfterTransmit) {
            return;
        }
    ...
    function onPhoneAppMessage(msg as Communications.PhoneAppMessage) as Void {
        ...
        processPhonePayload(payload, true);
    }

    function processPhonePayload(payload, shouldEmitHandshake) {
        if (payload == null) {
            emitSyncDiagnostic(
        ...
        if (messageType == "sync_request") {
            var reason = payload.hasKey("reason") ? payload["reason"] : "phone-request";
            if (shouldEmitHandshake) {
                requestSyncNow(reason);
            } else {
                requestSyncWithoutHandshake(reason);
            }
            return;
    ...
    function buildDeviceHelloMessage() {
        return {
            ...
            "deviceId" => GarminContract.DEVICE_ID,
            "deviceKind" => GarminContract.DEVICE_KIND,
            "deviceModel" => GarminContract.DEVICE_MODEL,
            "firmwareVersion" => GarminContract.buildFirmwareVersionString(),
            "appVersion" => GarminContract.APP_VERSION,
            "timezoneOffsetMinutes" => GarminContract.getTimezoneOffsetMinutes()
        };
    }
    ...
    function emitSyncDiagnostic(code, message, batchId) {
        if (!mHandshakeSent && !mPhoneReachable && !mShouldExitAfterTransmit) {
            return;
        }
    ...
    function buildTimestamp() {
        return GarminContract.buildUtcTimestamp();
    }
