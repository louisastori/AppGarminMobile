...
class MetricCollector {
    ...
    function captureBatch() {
        var timestamp = GarminContract.buildUtcTimestamp();
        ...
        return new BatchEnvelopeRecord(
            GarminContract.buildBatchId(sequence),
            sequence,
            timestamp,
    ...
    function buildLiveItems(timestamp, sequence) {
        ...
        appendMetricIfAvailable(
            items,
            timestamp,
            "temperature_c",
            live["temperature"],
            "C",
            GarminContract.SOURCE_SENSOR,
            GarminContract.QUALITY_DERIVED
        );
        appendMetricIfAvailable(
            items,
            timestamp,
        ...
        appendMetricIfAvailable(
            ...
            live["power"],
            "W",
            GarminContract.SOURCE_ACTIVITY,
            GarminContract.QUALITY_LIVE
        );
    ...
    function buildLivePreview(sequence) {
        ...
        var temperature = resolveTemperature(sensorInfo);
        var pressure = resolvePressure(sensorInfo);
        var heading = resolveHeading(sensorInfo);
        var power = resolveCurrentPower(activityInfo);
    ...
    function buildDailySnapshot(timestamp, sequence) {
        ...
        return new SnapshotRecord(
            GarminContract.buildSnapshotId(GarminContract.SNAPSHOT_DAILY, sequence),
    ...
    function buildMetric(timestamp, metricKey, metricValue, metricUnit, sourceDomain, quality) {
        ...
        return new MetricRecord(
            GarminContract.buildSampleId(mSampleSequence),
            timestamp,
            metricKey,
            metricValue,
            metricUnit,
    ...
    function appendMetricIfAvailable(items, timestamp, metricKey, metricValue, metricUnit, sourceDomain, quality) {
        if (metricValue == null) {
            return;
        }

        items.add(buildMetric(timestamp, metricKey, metricValue, metricUnit, sourceDomain, quality));
    }
    ...
    function resolveCurrentPower(activityInfo) {
    ...
    function resolveTimerState(activityInfo) {
        if (activityInfo != null && activityInfo.timerState != null) {
            return activityInfo.timerState.toString().toLower();
        }
    ...
    function resolveTemperature(sensorInfo) {
    ...
    function resolvePressure(sensorInfo) {
        if (sensorInfo != null && sensorInfo.pressure != null) {
            return sensorInfo.pressure;
        }

        return null;
    }

    function resolveHeading(sensorInfo) {
        if (sensorInfo != null && sensorInfo.heading != null) {
            return sensorInfo.heading;
        }
